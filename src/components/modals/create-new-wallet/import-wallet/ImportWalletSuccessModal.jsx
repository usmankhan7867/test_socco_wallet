...
function ImportWalletSuccessModal({
  isOpen,
  setIsOpen,
  isError,
  alertMessage,
}) {
  console.log("In Inananannananna:", isError, alertMessage);
  const navigate = useNavigate();
  function closeModal() {
    setIsOpen(false);
    if (!isError) navigate("/setup-wallet/set-password");
  }
  return (
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-[300px] transform overflow-hidden rounded-2xl bg-back px-6 py-8 text-left align-middle shadow-xl transition-all">
                <div
                  className={`flex justify-center items-center mb-4 w-20 h-20 mx-auto rounded-full ${
                    isError ? "bg-red-400" : "bg-btn"
                  }`}
                >
                ...
                <Dialog.Title
                  as="p"
                  className="text-lg font-semibold text-center text-white leading-6"
                >
                  {isError ? alertMessage : "Import succesfully!"}
                </Dialog.Title>
...
export default ImportWalletSuccessModal;
